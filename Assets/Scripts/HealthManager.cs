...
public class HealthManager : MonoBehaviour
{
    ...
    void Update()
    {
        if(invincCounter > 0)
        {
            ...
            for (int i = 0; i < PlayerControllerRobb.instance.playerPieces.Length; i++)
            {
                if (Mathf.Floor(invincCounter * 5f) % 2 == 0)
                {
                    PlayerControllerRobb.instance.playerPieces[i].SetActive(true);
                }
                else
                {
                    PlayerControllerRobb.instance.playerPieces[i].SetActive(false);
                }
    ...
    public void Hurt()
    {
        if (invincCounter <= 0)
        {
            ...
            {
                PlayerControllerRobb.instance.Knockback();
